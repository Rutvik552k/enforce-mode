...
console.log('enforce-skill-loader tests (v2 hardened)\n');
...
hookTest('.ts file → suggests code-review',
  { tool_name: 'Write', tool_input: {
    file_path: '/src/app.ts',
    content: 'export function hello() { return "world"; }\nconsole.log(hello());'
  }, transcript_path: '', session_id: SID + '-ext1' },
  true, { expectContains: 'ecc:code-review' });
clearState(SID + '-ext1');

hookTest('.py file → suggests python-review',
  { tool_name: 'Write', tool_input: {
...
hookTest('.go file → suggests go-review',
  { tool_name: 'Write', tool_input: {
...
hookTest('.rs file → suggests rust-review',
  { tool_name: 'Write', tool_input: {
...
hookTest('Auth code in .ts → security-review',
  { tool_name: 'Write', tool_input: {
    file_path: '/src/auth.ts',
    content: 'import bcrypt from "bcrypt";\nconst hash = await bcrypt.hash(password, 10);'
  }, transcript_path: '', session_id: SID + '-cnt1' },
  true, { expectContains: 'security-review' });
clearState(SID + '-cnt1');

hookTest('SQL queries → postgres-patterns',
  { tool_name: 'Edit', tool_input: {
    file_path: '/src/db.ts',
    new_string: 'const result = await db.query("SELECT * FROM users WHERE id = $1", [id]);'
  }, transcript_path: '', session_id: SID + '-cnt2' },
  true);
clearState(SID + '-cnt2');

hookTest('React hooks → senior-frontend',
  { tool_name: 'Write', tool_input: {
...
hookTest('LLM integration → ai-security',
  { tool_name: 'Write', tool_input: {
...
hookTest('Security research → security-review',
...
hookTest('Deployment research → senior-devops',
  { tool_name: 'WebSearch', tool_input: {
...
fs.writeFileSync(fakeTextTranscript, 'I will use ecc:code-review and ecc:tdd-workflow to review this code.\nThe Skill tool is great for this.');
...
// Create transcript with STRUCTURED skill invocation (should count)
const realTranscript = path.join(os.tmpdir(), 'enforce-sl-real-' + Date.now() + '.txt');
fs.writeFileSync(realTranscript, '{"tool_name":"Skill","tool_input":{"skill":"ecc:code-review"}}\nResult: skill loaded successfully');
...
// Create transcript with structured invocation of WRONG skill
const wrongSkillTranscript = path.join(os.tmpdir(), 'enforce-sl-wrong-' + Date.now() + '.txt');
fs.writeFileSync(wrongSkillTranscript, '{"tool_name":"Skill","tool_input":{"skill":"ecc:senior-architect"}}\nResult: skill loaded');

hookTest('ANTI-EVASION: wrong skill loaded → escalates (need code-review, have architect)',
  { tool_name: 'Write', tool_input: {
