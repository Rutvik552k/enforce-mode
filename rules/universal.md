...
- Constraint intake first (before design): real-world design is constraint-driven; without constraints a model defaults to hobby-grade. Before designing/implementing a feature or service, capture its production constraints in `constraints.json` at the project root (keyed by the same ids as dependency-map.json): SLO (p99 latency + availability + error budget), load (QPS, read/write ratio, data volume + growth), tenancy, data sensitivity, budget, and failure tolerance. A design with no SLO is not production-grade — STOP and ask the user for the values (AskUserQuestion) before proceeding. Never put secrets in `constraints.json`.
- Prior-art grounding (before design): the model's default design prior is tutorial/hobby code. Override it — for any non-trivial feature, identify the problem's algorithmic class, then research how ≥2 production systems (named companies) solved that class, with the named algorithm/architecture AND a real cited source (engineering blog / paper / talk), then reverse-engineer and adapt with stated Big-O. Record it in `design-dossier.md`. Hallucination guard: a "company X uses algorithm Y" claim with no citation is UNVERIFIED and must NOT be load-bearing in the design — verify it or drop it. Owned by research-solution-architect / research-agent; the design is built from the dossier.
